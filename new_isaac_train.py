...
CONFIG = {
    # --- 데이터 ---
    "data_dir":     "/home/kim/issac_sim_synth",   # 합성 데이터 폴더
    "save_dir":     "./checkpoints",                # 모델 저장 폴더
    "model_name":   "jun_resumed1.pth",             # 저장할 모델 파일명

    # --- 이어서 학습 (None이면 처음부터) ---
    "resume_from":  None,                            # 기존 pth 파일명 (save_dir 기준)

    # --- 학습 파라미터 ---
    "epochs":       150,
    "batch_size":   6,                               # AMP(BF16) 적용 시 6~8 권장 (5070 Ti 16GB)
    "lr":           1e-4,
    "img_size":     1280,
    "patience":     12,                              # Early stopping (val_loss 기준)
...
def main():
    ...
    if cfg["resume_from"]:
        ...
        if os.path.exists(resume_path):
            ...
            # Optimizer state도 복원 (Adam momentum 유지)
    ...
    # resume이면 기존 CSV에 이어쓰기, 아니면 새로 쓰고 헤더 작성
