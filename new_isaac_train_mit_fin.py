"""
Weld Bead Segmentation - Stage 2 Real-Image Fine-tuning (mit_b2)
- Backbone: MiT-B2 (SegFormer encoder) + U-Net decoder
- Base    : 합성 5300장으로 학습된 체크포인트 (train_synth_mitb2.py 산출물)
- Data    : 실이미지 ~250장 (rx160_image_01) 단독 fine-tune
- Task    : RGB(3ch) -> Mbead 이진 마스크

Usage:
  1) CONFIG["resume_from"] 에 합성 best.pth 경로 지정
  2) python train_finetune_real_mitb2.py

핵심 설계:
  - 합성 가중치 로드 후 낮은 LR로 미세조정 → catastrophic forgetting 방지
  - optimizer state는 복원하지 않음 (LR 체계가 바뀌므로 fresh optimizer)
  - epoch 카운터는 0부터 새로 시작 (fine-tune 스케줄 독립)
  - 실이미지 250장 → 강한 증강 + 긴 patience + 높은 weight_decay
...
# ==================================================================
CONFIG = {
    # --- 모델 (합성 base와 반드시 동일해야 load 성공) ---
    "backbone":         "mit_b2",
    "encoder_weights":  "imagenet",   # 어차피 resume_from으로 덮어씀 (구조 일치용)
    "decoder":          "unet",
    "in_channels":      3,
    "classes":          1,

    # --- 데이터 (실이미지) ---
    "data_dir":         "/home/kim/Desktop/rx160_image_01",   # ← 실이미지 ~250장 경로 (수정 필요)
    "rgb_glob":         "rgb_*.png",                          # ← 실제 파일명 패턴
    "mask_subdir":      "binary_masks",
    "mask_glob":        "mask_*.png",
    "save_root":        "./checkpoints",
    "exp_name":         "jun_resumed1_mitb2_finetune_real",

    # --- 합성 base 체크포인트 (★ 반드시 지정) ---
    "resume_from":      "./checkpoints/jun_resumed1_mitb2/best.pth",  # ← 합성 best.pth 경로

    # --- 학습 하이퍼파라미터 (fine-tune용) ---
    "epochs":           80,           # 작은 데이터 → 천천히 충분히
    "batch_size":       2,
    "accum_steps":      4,            # effective batch = 8
    "lr_encoder":       1e-5,         # ★ forgetting 방지: encoder 매우 보수적으로
    "lr_decoder":       5e-5,         # ★ 이미 학습된 decoder 미세조정
    "weight_decay":     0.03,         # 250장 과적합 방지 (합성 0.01 → 0.03 강화)
    "img_size":         1024,
    "warmup_epochs":    2,            # 사전학습됨 → warmup 최소화
    "patience":         25,           # 작은 데이터 → 길게
    "split_ratio":      (0.75, 0.15, 0.10),  # 250장: train 187 / val 37 / test 25
    "freeze_encoder_epochs": 0,       # 미세조정 → 처음부터 전체 학습
    "stronger_aug":     True,         # ★ 250장 → 강한 증강 필수
...
# ==================================================================
# 4. Augmentation
#   - 실이미지 250장 fine-tune: stronger=True 기본 (과적합 방지 + 현장 변이 모사)
# ==================================================================
...
def get_train_transform(img_size: int = 1024, stronger: bool = True):
    if stronger:
        return A.Compose([
...
# ==================================================================
# 9. Load synthetic base checkpoint (weights only)
# ==================================================================
def load_base_checkpoint(model, ckpt_path, device):
    """
    합성 base 체크포인트에서 model_state_dict만 로드.
    - optimizer state는 의도적으로 복원하지 않음 (LR 체계가 fine-tune용으로 바뀜)
    - strict=True 로 구조 불일치를 조기에 감지 (backbone/decoder가 base와 동일해야 함)
    """
    if not os.path.exists(ckpt_path):
        print(f"[ERROR] 합성 base 체크포인트를 찾을 수 없음: {ckpt_path}")
        print(f"        CONFIG['resume_from'] 경로를 확인하세요.")
        sys.exit(1)

    # PyTorch 2.6+ 는 weights_only 기본값이 True. 체크포인트에 numpy 스칼라(metric 등)가
    # 들어 있어 안전 로더가 거부하므로, 직접 만든 신뢰 가능한 파일이면 False로 명시.
    ckpt = torch.load(ckpt_path, map_location=device, weights_only=False)
    state = ckpt.get("model_state_dict", ckpt)
    missing, unexpected = model.load_state_dict(state, strict=True)
    print(f"\n[BASE] 합성 가중치 로드 완료: {ckpt_path}")
    print(f"  base epoch:    {ckpt.get('epoch', 'N/A')}")
    print(f"  base val_loss: {ckpt.get('val_loss', 'N/A')}")
    print(f"  base val_iou:  {ckpt.get('val_iou', 'N/A')}")
    print(f"  base val_dice: {ckpt.get('val_dice', 'N/A')}")
    print(f"  → optimizer state는 복원하지 않음 (fresh fine-tune optimizer)")
    print(f"  → epoch 카운터 1부터 새로 시작 (fine-tune 스케줄 독립)")
    return model
...
def main():
    ...
    print(f"\n{'='*60}\nFine-tune (mit_b2, real) — {cfg['exp_name']}\n{'='*60}")
    ...
    # --- ★ 합성 base 가중치 로드 ---
    model = load_base_checkpoint(model, cfg["resume_from"], device)

    total_params = sum(p.numel() for p in model.parameters())
    print(f"Total params: {total_params:,}")

    criterion = BCEDiceLoss()
    optimizer = build_optimizer(model, cfg)   # fresh optimizer (fine-tune LR)
