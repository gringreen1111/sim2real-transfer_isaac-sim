"""
Weld Bead Segmentation - Real-Image Only Training (mit_b2)
========================================================================
Architecture : U-Net decoder + MiT-B2 (SegFormer) encoder, ImageNet pretrained
Data         : 실이미지 ~251장 (rx160_image_01) 단독 학습 (resume 없음)
Input        : RGB 3-channel, 1024 x 1024 (longest-side resize + pad)
Loss         : BCEWithLogitsLoss + Dice Loss
Optimizer    : AdamW (differential LR: encoder 3e-5 / decoder 2e-4) + warmup-cosine
Threshold    : 0.4 (참고용)
Mixed Prec.  : BF16 autocast (5070 Ti / Ada Lovelace)

사용법: 아래 CONFIG 값 수정 후 → python3 train_onlyreal_mitb2.py

주의:
  - resume_from = None → ImageNet pretrained encoder부터 처음부터 학습.
  - 실이미지 251장 단독이라 강한 증강 + 높은 weight_decay로 과적합 방지.
  - MiT-B2는 SCSE attention을 쓰지 않음(Transformer 자체 self-attention 내장).

출력물 (save_dir/exp_name/ 아래):
...
CONFIG = {
    ...
    # --- 데이터: 실이미지 단독 학습 ---
    "data_dir":         "/home/kim/Desktop/rx160_image_01",   # 실이미지 폴더 (~251장)
    "save_dir":         "./checkpoints",
    "exp_name":         "jun_onlyreal_mitb2",                  # 결과물 하위 폴더명
    "model_name":       "jun_onlyreal_mitb2.pth",             # ★ 저장할 모델 파일명

    # --- resume (단독 학습이므로 None) ---
    "resume_from":      None,                                  # ★ 처음부터 학습

    # --- 학습 하이퍼파라미터 (251장 단독) ---
    "epochs":           120,          # 데이터 적으므로 충분히 길게
    "batch_size":       2,
    "accum_steps":      4,            # effective batch = 8 (Transformer는 큰 batch 선호)
    "lr_encoder":       3e-5,         # pretrained encoder 보존
    "lr_decoder":       2e-4,         # decoder는 scratch → 높게
    "weight_decay":     0.03,         # 251장 과적합 방지 (Transformer 필수)
    "img_size":         1024,
    "warmup_epochs":    8,            # 데이터 적으니 천천히 시작
    "patience":         30,           # 작은 데이터 → val 노이즈 큼 → 여유
    "split_ratio":      (0.75, 0.15, 0.10),  # 251장: train 188 / val 37 / test 26
    "freeze_encoder_epochs": 2,       # 초반엔 decoder만 적응
    "stronger_aug":     True,         # 251장 단독 → 강한 증강 필수
...
def _safe_pad(img_size):
    """PadIfNeeded: albumentations 버전별 인자명 차이 자동 분기 (value→fill)"""
    try:
        return A.PadIfNeeded(min_height=img_size, min_width=img_size,
    ...
    if stronger:
        return A.Compose([
            ...
            A.CoarseDropout(num_holes_range=(1, 4),
                            ...
                            hole_width_range=(30, 100), fill=0, p=0.25),
...
# ==================================================================
# 7. Train / Validate (AMP, gradient accumulation)
# ==================================================================
def train_one_epoch(model, loader, criterion, optimizer, scheduler,
                    device, amp_dtype, accum_steps):
    model.train()
    total_loss = 0.0
    total_iou = 0.0
...
def collect_pairs(cfg):
    ...
    mask_dir = os.path.join(cfg["data_dir"], "binary_masks")
    mask_paths_all = glob.glob(os.path.join(mask_dir, "mask_*.png"))
...
def main():
    ...
    if cfg["use_amp"] and device.type == "cuda":
        if cfg["amp_dtype"] == "bfloat16":
            ...
            print("AMP: enabled (FP16) — Note: GradScaler not used. "
                  "Consider BF16 on Ada Lovelace GPU.")
        else:
    ...
    if cfg["resume_from"]:
        ...
        if os.path.exists(resume_path):
            ...
            print(f"[RESUME] 가중치 로드: {resume_path}")
        else:
            print(f"[WARNING] resume_from 파일 없음: {resume_path} → ImageNet부터 학습")
    else:
        print(f"[INFO] resume_from = None → ImageNet pretrained encoder부터 처음부터 학습")
    ...
    for epoch in range(1, cfg["epochs"] + 1):
        ...
        train_loss, train_iou = train_one_epoch(
            ...
            device, amp_dtype, cfg["accum_steps"])
        ...
        if val_loss < best_val_loss:
            ...
            best_epoch = epoch
            no_improve = 0
            save_path = os.path.join(exp_dir, cfg["model_name"])
            torch.save({
                "epoch": epoch,
            ...
            print(f"  *** Best saved! Val Loss: {val_loss:.4f}, "
                  f"IoU: {val_iou:.4f}, Dice: {val_dice:.4f} → {save_path} ***")
        else:
            no_improve += 1
    ...
    print(f"  Model saved:   {os.path.join(exp_dir, cfg['model_name'])}")
