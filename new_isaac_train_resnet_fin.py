...
# ==================================================================
CONFIG = {
    # --- 데이터: 실제 D405 이미지로 fine-tune ---
    "data_dir":     "/home/kim/Desktop/rx160_image_01",     # 실제 이미지 폴더 (~251장)
    "save_dir":     "./checkpoints",                # 모델 저장 폴더
    "model_name":   "jun_finetuned1.pth",            # ★ 새 이름 (합성 best 덮어쓰기 방지)

    # --- Fine-tune: 합성 best 모델에서 가중치 이어받기 ---
    "resume_from":  "jun_resumed1.pth",             # 합성-only best 체크포인트

    # --- 학습 파라미터 (fine-tune용 조정) ---
    "epochs":       80,                              # 데이터 적으니 축소
    "batch_size":   6,                               # 1280 해상도 + 안정성
    "lr":           1e-4,                            # ★ 실측상 1e-5보다 1e-4가 우수
    "img_size":     1280,
    "patience":     15,                              # 데이터 적어 val 노이즈 큼 → 여유
    ...
    # --- Fine-tune 옵션 ---
    "reset_lr":     True,                            # resume 시 optimizer lr을 CONFIG lr로 초기화
    "reset_epoch":  True,                            # 에폭 카운트를 1부터 새로 시작 (다른 데이터셋)
    "load_optimizer": False,                         # ★ optimizer state 복원 안 함 (fresh Adam)
...
# ==================================================================
# 4. Augmentation (합성→실제 도메인 갭 완화 위해 강화)
# ==================================================================
...
def get_train_transform(img_size: int = 1280):
    """
    합성 데이터로 학습 후 실제 D405 이미지에 적용하기 위한
    aggressive augmentation. 도메인 갭 완화에 결정적.
    """
    return A.Compose([
        # --- 리사이즈 (aspect 보존) ---
...
def train_one_epoch(model, loader, criterion, optimizer, device,
    ...
    for batch_idx, (images, masks) in enumerate(loader):
        ...
        if use_amp:
            ...
            # BF16은 GradScaler 불필요 (FP16과의 큰 차이점)
            loss.backward()
            optimizer.step()
        else:
            outputs = model(images)
...
def main():
    ...
    split_path = os.path.join(cfg["save_dir"], "split_finetune.json")
    ...
    # --- 합성 best 체크포인트에서 가중치 로드 (Fine-tune) ---
    start_epoch = 1
    if cfg["resume_from"]:
        ...
        if os.path.exists(resume_path):
            ...
            # 모델 가중치는 항상 복원 (fine-tune의 출발점)
            model.load_state_dict(checkpoint["model_state_dict"])
            print(f"\n[FINE-TUNE] 가중치 로드 완료: {resume_path}")

            # Optimizer state 복원 여부 (fine-tune 기본: 복원 안 함 → fresh Adam)
            if cfg.get("load_optimizer", False) and "optimizer_state_dict" in checkpoint:
                optimizer.load_state_dict(checkpoint["optimizer_state_dict"])
                print(f"  Optimizer state 복원됨")
            else:
                print(f"  Optimizer 새로 시작 (fresh Adam, momentum 초기화)")

            # lr 초기화: scheduler가 깎아놨던 lr 무시하고 CONFIG lr로 강제
            if cfg.get("reset_lr", True):
                for g in optimizer.param_groups:
                    g["lr"] = cfg["lr"]
                print(f"  lr 초기화: {cfg['lr']:.0e}")

            # 에폭 카운트: fine-tune은 다른 데이터셋이라 1부터 새로 시작
            if cfg.get("reset_epoch", True):
                start_epoch = 1
                print(f"  Epoch 카운트 1부터 시작 (새 데이터셋)")
            else:
                start_epoch = checkpoint.get("epoch", 0) + 1
                print(f"  Epoch {start_epoch}부터 이어서")

            print(f"  [참고] 합성 모델 성능 - "
                  f"IoU: {checkpoint.get('val_iou', 'N/A')}, "
                  f"Dice: {checkpoint.get('val_dice', 'N/A')}")
        else:
            print(f"\n[ERROR] resume_from 파일 없음: {resume_path}")
            print(f"  → Fine-tune은 합성 best 체크포인트가 반드시 필요합니다. 종료합니다.")
            sys.exit(1)
    else:
        print(f"\n[WARNING] resume_from = None → Fine-tune인데 베이스 모델이 없습니다!")
        print(f"  → ImageNet pretrained로 처음부터 학습됩니다 (의도와 다를 수 있음).")
    ...
    # --- CSV 로그 초기화 (fine-tune 전용 파일) ---
    csv_path = os.path.join(cfg["save_dir"], "finetune_log.csv")
    csv_fields = ["epoch", "train_loss", "train_iou",
                  "val_loss", "val_iou", "val_dice", "lr"]
    # fine-tune은 epoch 1부터 새로 시작 → 항상 새로 쓰고 헤더 작성
    ...
    for epoch in range(start_epoch, cfg["epochs"] + 1):
        ...
        # History 저장 (매 에폭)
        history_path = os.path.join(cfg["save_dir"], "history_finetune.json")
